...
export default class MovieTemplate {
  ...
  readonly setMovies = (movies: Movie[]): void => {
    this.movies = movies
  }
  ...
  private readonly getStarsHTML = (score: number): string => {
    const stars = []
  ...
  readonly getMovieHTML = (movie: Movie): string => {
    ...
    <div class="movie">
        <div class="movie-poster">
          <img src="./asset/img/${movie.thumbnail}" alt="${movie.title}" onerror="this.src='./asset/img/not-icon.png'">
          <div>
            <p>
              ${this.getStarsHTML(movie.score)}
            </p>
            <ul>
        ...
        <div class="movie-info">
          <div>
            ...
            <div class="movie-btn-rental">
              <button>$${movie.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} RENT</button>
            </div>
          </div>
        </div>
      </div>
